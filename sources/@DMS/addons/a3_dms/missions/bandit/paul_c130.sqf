...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_c130.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	...
	case "difficult":
	{
		...
		_crate_weapons 		= (16 + (round (random 5)));
	...
	default
	{
		...
		_crate_items 		= (15 + (round (random 7)));
		_crate_backpacks 	= (10 + (round (random 5)));
	};
};

_msgStart = ['#FFFF00',"A C130 has been shot down! Go kill the soldiers and grab the loot!"];
...
if (_difficulty isEqualTo "easy") then
{
	...
	_msgWIN = ['#0080ff',"The crashed soldiers have been eliminated!"];
}
else
{
	_pinCode = (1000 +(round (random 8999)));
	_vehicle = [_VehicleClass,[(_pos select 0) -20, (_pos select 1) -20, 0],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
	_msgWIN = ['#0080ff',format ["The crashed soldiers have been eliminated, entry code %1...",_pinCode]];
};
...
_msgLOSE = ['#FF0000',"The Soldiers have fixed the C130 and flew away..."];

// Define mission name (for map marker and logging)
_missionName = "Crashed C130";
